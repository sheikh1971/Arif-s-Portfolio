...
Key Results: 
• Achieved 88.55% test accuracy on the full dataset, significantly outperforming all baseline models (best baseline: 51.56%). 
• Ablation study demonstrated an ~9% accuracy improvement attributable solely to SE blocks versus an identical baseline CNN. 
• Delivered strong discriminative performance with AUC > 0.90 for major disease classes, supported by detailed confusion matrices and ROC analysis. 
• Maintained a moderate parameter count (~5M), balancing performance with deployability for telemedicine and mobile health platforms.
